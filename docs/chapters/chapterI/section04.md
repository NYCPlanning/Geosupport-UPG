...
<u>In general, anything that applies to a basic Geosupport Function will also apply to the Extended Version of that function and other variations (e.g. Long Work Area or Auxiliary Segments) as well.</u>  For example, anything that is true of Function 1 will also be true of Function 1 Extended.  In our discussion of the various functions, for simplicity, we will often refer only to the basic function, but the information will apply also to the Extended function and other variations.
