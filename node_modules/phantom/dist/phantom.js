...
function createLogger({ logLevel = defaultLogLevel } = {}) {
  const { format } = _winston2.default;
  return _winston2.default.createLogger({
    transports: [new _winston2.default.transports.Console({
      level: logLevel,
      format: format.combine(_winston2.default.format.colorize(), format.splat(), format.simple())
    })]
  });
}
...
class Phantom {
  ...
  constructor(args = [], {
    ...
    if (typeof shimPath !== 'string') {
      throw new Error('Path to shim file was not found. ' + 'Are you sure you entered the path correctly? Exiting.');
    }
